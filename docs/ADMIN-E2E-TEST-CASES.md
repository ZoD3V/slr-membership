...
- Halaman Info Hadiah — ✅ DIKOREKSI, bukan gap. Halaman member (`/member/prizes`) ternyata SUDAH tersambung penuh sejak awal (catatan lama di admin salah, sudah diperbaiki). Halaman marketing publik `/prizes` memang sengaja tidak pakai CMS ini — sesuai desain, endpoint-nya cuma untuk member yang login, halaman publik punya konten marketing statis sendiri.
...
3. ✅ DIPERBAIKI 2026-08-16 — Tambah e-book baru dulu gagal dengan pesan error umum yang tidak menyebutkan field mana yang bermasalah (ternyata Gambar Sampul). Sekarang dicek di sisi tampilan sebelum kirim, pesannya langsung jelas.
