...
| 5 | Simpan promo baru. | Muncul di daftar, notifikasi sukses. | ✅ 2026-08-17: DIPERBAIKI. Promo tersimpan dan notifikasi sukses muncul dengan benar. Sebelumnya ditemukan bug nyata (bukan cuma salah amat): halaman gagal pindah ke daftar promo setelah simpan — tombol macet, admin tidak dapat kepastian apakah data tersimpan. Sudah diperbaiki dan dites ulang di server sungguhan: sekarang langsung pindah ke daftar promo begitu simpan berhasil. |
...
| 9 | Edit e-book yang sudah ada (baik jenis Bab-bab maupun PDF), cek Tingkat Akses. | **Perhatikan baik-baik:** tingkat akses akan kembali ke pengaturan default (RED) setiap kali dibuka untuk edit — HARUS dipilih ulang secara manual sebelum simpan, kalau tidak bisa salah mengubah akses e-book tanpa disadari admin. | 🚧 2026-08-17: DIKONFIRMASI via dokumentasi API resmi — bukan bug FE, backend memang tidak pernah mengirim tingkat akses e-book lewat endpoint manapun (baik daftar maupun detail), jadi form tidak punya cara tahu nilai aslinya. Sudah ditambahkan peringatan tetap (bukan cuma info hover) di bawah pilihan Tingkat Akses saat edit, supaya admin tidak mungkin melewatkannya. Perlu backend menambahkan field ini ke response GET supaya benar-benar teratasi. |
...
| 6 | Cek konsistensi data lintas halaman — misalnya jumlah member di Beranda vs jumlah member di halaman Members. | Angka harus sama/konsisten. | ⚠️ 2026-08-17: Digali lebih lanjut. (1) Kartu BENY di Beranda sudah benar (lihat Suite 1 langkah 2). (2) Ditemukan bug kosmetik terpisah: 2 akun yang punya add-on BENY tampil dengan Tier "Smart Life Rewards Add On - BENY - DAILY" di tabel Members, padahal tier asli mereka Visitor — SUDAH DIPERBAIKI (frontend). Selisih total (66 di kartu ringkasan vs "Total: 65 entries" di tabel) DIKONFIRMASI bukan soal waktu — sudah diisolasi persis ke grup Visitor (statistik bilang 14, tabel cuma tampilkan 13; grup Red dan Blue sama persis, tidak ada selisih). Artinya ada 1 data keanggotaan berstatus Visitor yang dihitung backend di statistik tapi tidak muncul sama sekali di endpoint daftar member manapun — kemungkinan akun yatim/tidak tertaut dengan benar di sisi backend. Belum bisa diperbaiki dari frontend, perlu backend menelusuri data anggota Visitor yang hilang dari daftar. |
...
2. ✅ DIPERBAIKI 2026-08-17 — Simpan data Promo Partner (Suite 5). Notifikasi sukses memang muncul (sama seperti temuan #1), TAPI ditemukan bug nyata terpisah: halaman gagal pindah ke daftar promo setelah simpan — tombol simpan macet (nonaktif permanen), admin tidak dapat kepastian data tersimpan meski sebenarnya sudah tersimpan di server. Akar masalah: dua perintah pindah-halaman dipanggil berurutan tanpa jeda, saling bentrok dan macet. Sudah diperbaiki (hapus satu perintah yang berlebihan) dan dites ulang langsung di server sungguhan — sekarang pindah halaman normal begitu simpan berhasil.
...
6. ✅ DIPERBAIKI 2026-08-17 (backend + frontend) — Kartu "Members by Sub-Tier" dulu duplikat baris "Visitor" dan totalnya meleset jauh (68 vs 65). Backend membenahi sumber datanya (add-on BENY tidak lagi ikut ke-groupBy sebagai tier), frontend menyesuaikan ke bentuk data baru.
7. ✅ DIPERBAIKI 2026-08-17 (frontend) — 2 akun ber-add-on BENY tampil dengan Tier "Smart Life Rewards Add On - BENY - DAILY" (bukan "Visitor") di tabel Members. Ditemukan saat menelusuri temuan #6. Akar masalah: kode pencocokan nama tier butuh nama pemasaran yang cocok persis, padahal grup Visitor cuma punya 1 kemungkinan tier — sekarang langsung dipastikan "Visitor" untuk grup itu tanpa perlu pencocokan nama.
8. 🚧 BELUM SELESAI (backend) — Selisih 66 vs 65 di temuan #6 DIKONFIRMASI bukan soal waktu: diisolasi persis ke grup Visitor (statistik 14, tabel cuma 13; Red & Blue sama persis). ada 1 data keanggotaan Visitor yang terhitung di statistik tapi tidak muncul di endpoint daftar member manapun. Perlu backend menelusuri data yang hilang ini.
...
**Verdict penutup:** Dashboard admin secara keseluruhan berfungsi baik dan aman digunakan — role-gate admin/member teruji dua arah, kotak konfirmasi hapus konsisten di semua halaman, dan tidak ada aksi yang menghapus/mengubah data tanpa konfirmasi. Semua 6 Known Gap dan 7 dari 8 temuan baru sudah selesai diperbaiki atau dikoreksi (gabungan kerja frontend + backend, semuanya diverifikasi ulang langsung lewat API dan halaman live, bukan cuma baca kode). Sisa 1 item terbuka murni di sisi backend (lihat temuan #8) — 1 data keanggotaan Visitor hilang dari endpoint daftar member meski terhitung di statistik; tidak berbahaya bagi member/admin, tapi perlu ditelusuri backend.
