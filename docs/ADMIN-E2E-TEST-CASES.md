...
| **Hapus bab pada e-book** | 🚧 | Kotak konfirmasi untuk hapus bab tampilannya beda (kotak konfirmasi bawaan browser) dari kotak konfirmasi standar yang dipakai di seluruh halaman admin lainnya. Bukan masalah fungsi, hanya tidak konsisten tampilannya. |
...
| 2 | Cek bagian ringkasan BENY. | Tampil 4 angka: Menunggu Aktivasi, Aktif, Menunggu Nonaktif, Dibatalkan. | ⚠️ **Masalah ditemukan** — angka "Menunggu Aktivasi" ter-update dengan benar dan real-time (dicek ulang setelah aksi di Suite BENY, angkanya ikut berubah). Tapi angka "Aktif" selalu menunjukkan 0, padahal saat dicek langsung ke halaman BENY menu Aktif ada 9 akun yang benar-benar aktif. Ringkasan di beranda ini salah/tidak sinkron untuk kategori Aktif. |
...
| 6 | Konfirmasi penyimpanan. | Pemenang tersimpan, muncul di daftar, notifikasi sukses. | ⚠️ **Masalah ditemukan** — saat undian yang dipilih tanggal tariknya masih di MASA DEPAN, penyimpanan gagal (sistem memang benar menolak karena undian belum resmi ditarik), TAPI layar tidak menampilkan pesan error apapun ke admin — terlihat seperti tidak terjadi apa-apa, tanpa penjelasan kenapa gagal. Membingungkan buat admin yang tidak tahu penyebabnya. Untuk undian yang tanggal tariknya sudah lewat, penyimpanan berhasil normal dengan notifikasi sukses. |
...
| 5 | Simpan promo baru. | Muncul di daftar, notifikasi sukses. | ⚠️ **Masalah ditemukan** — promo BERHASIL tersimpan dan muncul di daftar, tapi tombol simpan macet di kondisi nonaktif tanpa pindah halaman dan TANPA notifikasi sukses apapun. Terlihat seperti gagal/macet padahal sebenarnya berhasil — berpotensi bikin admin klik tombol simpan berkali-kali karena mengira belum tersimpan. |
...
| 7 | Edit promo, ubah status Unggulan. | Perubahan tersimpan dan tampil sesuai di sisi member (promo unggulan biasanya tampil lebih menonjol). | ⚠️ Perubahan tersimpan dengan benar (dicek ulang di daftar, status berubah jadi "Ya"), tapi masalah yang sama seperti langkah 5 terjadi lagi — tombol macet tanpa notifikasi setelah simpan. |
...
| 4 | Buat e-book jenis Bab-bab. Isi Judul, Subjudul, Deskripsi, Sampul, Kategori, Estimasi Baca, dan pilih Tingkat Akses (Visitor/Red/Blue). | Semua isian wajib terisi, tersimpan dengan benar. | ⚠️ **Ditemukan celah kecil** — pesan error saat isian wajib belum lengkap cuma bilang "ada isian yang tidak valid" secara umum, TANPA menyebutkan isian mana yang kurang. Sempat 2x percobaan simpan gagal sebelum ketahuan bahwa Sampul (gambar) yang belum diisi adalah penyebabnya. Setelah semua isian lengkap (termasuk Sampul), berhasil tersimpan dengan baik. |
...
| 4 | Gunakan filter Jenis dan Status. | Tabel tersaring sesuai pilihan. | ⚠️ Filter berfungsi (dicoba filter jenis "otp", tabel tersaring jadi 16 hasil, semua memang jenis otp). Tapi ditemukan celah kecil: pilihan di dropdown filter Jenis cuma ada 3 (welcome/otp/password_reset), padahal riwayat sungguhan punya jenis lain juga (mis. "email_verification") yang tidak bisa dipilih di filter ini. |
...
| 2 | Cek semua aksi Hapus di seluruh halaman admin (Member, Undian, Pemenang, Promo, E-Book). | SELALU ada kotak konfirmasi sebelum data benar-benar terhapus — tidak ada yang langsung terhapus tanpa konfirmasi. | ⚠️ — hampir semua halaman (Member, Undian, Promo, E-Book) selalu menampilkan kotak konfirmasi bergaya seragam sebelum hapus sungguhan terjadi. Satu pengecualian yang sudah tercatat di Known Gaps: hapus bab pada E-Book memakai kotak konfirmasi bawaan browser (tampilannya beda/kurang rapi), tapi konfirmasi tetap ada, jadi tidak ada risiko data terhapus tanpa sengaja. |
| 3 | Cek notifikasi sukses/gagal di setiap aksi simpan/hapus/kirim di seluruh halaman admin. | Selalu ada pesan yang jelas (bukan diam saja atau halaman macet) baik saat berhasil maupun gagal. | ⚠️ — sebagian besar halaman sudah baik. Namun ditemukan 2 celah nyata: (1) mencatat pemenang pada undian yang jadwal undinya belum lewat gagal total tanpa pesan apa pun ke admin (lihat Suite 4) — admin tidak tahu kenapa gagal, dan (2) simpan/edit data Promo kadang terlihat macet (tombol tetap nonaktif, tidak ada pesan sukses) padahal perubahan sebenarnya sudah tersimpan (lihat Suite 5) — berisiko admin mengulang aksi karena mengira gagal. |
| 4 | Cek fitur pencarian & filter di setiap halaman yang punya tabel data. | Semua berfungsi dan hasilnya sesuai dengan kata kunci/filter yang dipilih. | ⚠️ — pencarian & filter berfungsi baik di semua halaman yang dicoba (Member, Undian, Promo, Riwayat Notifikasi, Ekspor Data Undian). Satu celah kecil sudah tercatat di Suite 11: pilihan filter Jenis pada Riwayat Pengiriman Notifikasi tidak lengkap (cuma 3 dari beberapa jenis pesan yang sebenarnya ada). |
...
| 6 | Cek konsistensi data lintas halaman — misalnya jumlah member di Beranda vs jumlah member di halaman Members. | Angka harus sama/konsisten. | ❌ Ditemukan 2 masalah nyata. (1) Sudah tercatat di Suite 1/7: kartu ringkasan BENY "Active Accounts" di Beranda selalu menunjukkan 0, padahal tab Active di halaman BENY berisi 9 akun aktif sungguhan. (2) Baru ditemukan: jumlah total member di Beranda (65) memang cocok dengan "Total: 65 entries" di halaman Members, TAPI kartu ringkasan "Members by Sub-Tier" di bagian atas halaman Members sendiri menunjukkan total 68 dan baris "Visitor" muncul DUA KALI dengan angka berbeda (14 dan 2) alih-alih digabung jadi satu — jumlah breakdown-nya tidak cocok dengan jumlah member sungguhan di tabel bawahnya. |
...
**3 Known Gap lama masih berlaku, belum diperbaiki** (bukan bug baru, sudah tercatat sejak awal): Halaman Info Hadiah belum tersambung ke tampilan member/publik, tab "Menunggu Nonaktif" BENY memakai cara ambil-data yang kurang efisien, kotak konfirmasi hapus bab e-book tampilannya beda dari kotak konfirmasi standar.
...
2. Simpan/edit data Promo Partner kadang terlihat macet (tombol tetap nonaktif, tidak ada pesan sukses) padahal perubahan sebenarnya sudah tersimpan (Suite 5).
3. Tambah e-book baru gagal dengan pesan error umum yang tidak menyebutkan field mana yang bermasalah — ternyata Gambar Sampul yang wajib diisi (Suite 6).
4. Kartu ringkasan "Active Accounts" BENY di Beranda selalu menunjukkan 0, padahal tab Active di halaman BENY berisi akun aktif sungguhan (Suite 1 & 7).
5. Pilihan filter Jenis pada Riwayat Pengiriman Notifikasi tidak lengkap — cuma menampilkan 3 dari beberapa jenis pesan yang sebenarnya ada di riwayat (Suite 11).
6. Kartu ringkasan "Members by Sub-Tier" di halaman Members menampilkan total (68) yang tidak cocok dengan jumlah member sungguhan (65), dan baris "Visitor" muncul dua kali dengan angka terpisah alih-alih digabung jadi satu (Cross-Cutting Check 6).
...
**Verdict penutup:** Dashboard admin secara keseluruhan berfungsi baik dan aman digunakan — role-gate admin/member teruji dua arah, kotak konfirmasi hapus konsisten di hampir semua halaman, dan tidak ada aksi yang menghapus/mengubah data tanpa konfirmasi. Temuan yang ada bersifat gap pada pesan error/notifikasi dan konsistensi angka ringkasan di beberapa kartu statistik — bukan masalah yang mengancam integritas data, tapi tetap membingungkan admin kalau dibiarkan.
