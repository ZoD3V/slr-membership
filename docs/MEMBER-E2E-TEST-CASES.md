...
| 6 | Klik "Next Ebook" di akhir. | Navigasi ke e-book berikutnya. | ⚠️ **Deviasi dari spec** — tombol aktual berlabel "More E-Books" (`ebook-reader.tsx` via `[id]/page.tsx:164-165`, `nextHref` di-hardcode ke `/member/ebooks`), bukan navigasi ke e-book spesifik berikutnya. Berfungsi (nav ke listing), tapi bukan behavior "Next Ebook" sesuai deskripsi PRD. |
| 7 | Cari tombol download/offline. | TIDAK ADA — no download/offline feature di web (mobile-only). | ✅ **FIXED (2026-08-15)** — ditemukan 2 e-book lama berformat PDF ("React JS Ebooks", "Next JS Ebook", `chapter_count:0`) di-render lewat `PdfEbookViewer` yang punya tombol **"Download"** eksplisit (`download` attribute) — melanggar rule CLAUDE.md §1. Fixed: dua link `download` dihapus dari `pdf-ebook-viewer.tsx`, sisa "Open PDF" (buka tab baru, tanpa save-as paksa). Type-check clean. |
