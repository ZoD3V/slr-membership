 ...
 * exposes no state filter, so that half of the pool is narrowed here.
 */
export async function searchWinnerMembersAction(query: WinnerMemberQuery): Promise<MemberSearchResult> {
    ...
    try {
        ...
        return {
            ok: true,
            data: scoped.map((m) => {
                // The list row carries only a marketing name ('Plus'); the group is
                // known from the tier we queried, which pins the exact sub-tier.
                const code = subTierFromGroupAndName(query.tier, m.tier);
