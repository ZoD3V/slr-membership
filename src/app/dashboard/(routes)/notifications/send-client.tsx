                    ...
                    <CardContent className='space-y-6 px-0 pb-0'>
                        {templatesArePlaceholders ? (
                            <p className='text-muted-foreground flex items-start gap-2 text-sm'>
                                ...
                                <span>
                                    Sending is unavailable — the template list couldn&apos;t be loaded, so the options
                                    below are placeholders and don&apos;t exist on the server.
                                </span>
                            </p>
                        ) : hasNoTemplates ? (
                            <p className='text-muted-foreground flex items-start gap-2 text-sm'>
                                <TriangleAlert className='mt-0.5 size-4 shrink-0 text-amber-400/70' />
                                <span>
                                    Sending is unavailable — the platform has no notification templates, and the send
                                    API requires a template id. Add one before sending.
                                </span>
                            </p>
                        ) : null}
                                ...
                                {recipients.map((r) => (
                                    <span
                                        ...
                                        <button
                                            ...
                                            onClick={() =>
                                                setRecipients((current) =>
                                                    current.filter((c) => c.user_id !== r.user_id)
                                                )
                                            }>
                                            <X className='size-3' />
                                        </button>
